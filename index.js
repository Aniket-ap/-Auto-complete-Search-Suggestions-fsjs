...
input.addEventListener("input", (event) => {
  ...
  if (event.target.value) {
    filteredArray = cars.filter((item) => {
      return item.name.toLowerCase().includes(event.target.value);
    });
    filteredArray = filteredArray.map(
      (car) => `<li onclick="setValue('${car.name}')">${car.name}</li>`
    );
  }

  showCarsNameInList(filteredArray);
});

function showCarsNameInList(CarsArr) {
  if (CarsArr.length > 0) {
    input.style.borderRadius = "4px 4px 0 0";
  }
  const html = !CarsArr.length ? "" : CarsArr.join("");
